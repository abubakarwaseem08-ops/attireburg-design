...
export default function ProductDetail() {
  ...
  const [selectedFit, setSelectedFit] = useState<'slim' | 'loose' | null>(null)
  const [showSizeChart, setShowSizeChart] = useState<'slim' | 'loose' | null>(null)
              ...
              {getCurrentImages().length > 1 && (
                <>
                  <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 hover:bg-white shadow flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/90 hover:bg-white shadow flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                  ...
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                    {getCurrentImages().map((_, i) => (
                      <button
                        ...
                        className={`w-1.5 h-1.5 rounded-full transition-colors ${selectedImage === i ? 'bg-gray-900' : 'bg-gray-400'}`}
                      />
                    ))}
                  </div>
                </>
          ...
          <div className="space-y-6">
            ...
            {product.hasVariants && product.variants ? (
              ...
              <div className="space-y-4">
                {/* Fit Selection */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-3">Passform / Fit</h3>
                  <div className="flex gap-2">
                    {(['slim', 'loose'] as const).map((fit) => (
                      <button
                        key={fit}
                        onClick={() => {
                          setSelectedFit(fit)
                          setShowSizeChart(fit)
                        }}
                        className={`px-3 py-1.5 border text-xs font-medium tracking-wide uppercase transition-colors ${
                          selectedFit === fit
                            ? 'border-gray-900 bg-gray-900 text-white'
                            : 'border-gray-300 text-gray-600 hover:border-gray-900 hover:text-gray-900'
                        }`}
                      >
                        {fit === 'slim' ? 'Slim Fit' : 'Loose Fit'}
                      </button>
                    ))}
                  </div>
                  {selectedFit && (
                    <p className="text-xs text-gray-500 mt-2">
                      Ausgewählt: <span className="font-medium text-primary-600">{selectedFit === 'slim' ? 'Slim Fit' : 'Loose Fit'}</span>
                      {' '}—{' '}
                      <button onClick={() => setShowSizeChart(selectedFit)} className="underline hover:text-primary-600">
                  ...
                  >
                    ...
                    >
                      ...
                      <h3 className="text-xl font-bold text-gray-900 mb-1">
                        {showSizeChart === 'slim' ? 'Slim Fit' : 'Loose Fit'} — Größentabelle
                      </h3>
                      <p className="text-sm text-gray-500 mb-5">Alle Maße in Zentimetern (cm)</p>

                      {showSizeChart === 'slim' ? (
                        <table className="w-full text-sm border-collapse">
                          <thead>
                            <tr className="bg-primary-600 text-white">
                              <th className="py-2 px-3 text-left rounded-tl-lg">Größe</th>
                              <th className="py-2 px-3 text-center">Brust</th>
                              <th className="py-2 px-3 text-center">Taille</th>
                              <th className="py-2 px-3 text-center rounded-tr-lg">Länge</th>
                            </tr>
                          </thead>
                          <tbody>
                            {[
                              ...
                              ['L', '94–98', '82–86', '74'],
                              ['XL', '98–104', '86–92', '76'],
                              ['XXL', '104–110', '92–98', '78'],
                            ].map(([size, chest, waist, length], i) => (
                              <tr key={size} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                                <td className="py-2 px-3 font-semibold text-primary-700">{size}</td>
                                <td className="py-2 px-3 text-center text-gray-700">{chest}</td>
                                <td className="py-2 px-3 text-center text-gray-700">{waist}</td>
                                <td className="py-2 px-3 text-center text-gray-700">{length}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                        ...
                        <table className="w-full text-sm border-collapse">
                          ...
                          <tbody>
                            {[
                              ...
                              ['M', '98–104', '88–94', '74'],
                              ['L', '104–110', '94–100', '76'],
                              ['XL', '110–118', '100–108', '78'],
                              ['XXL', '118–126', '108–116', '80'],
                            ].map(([size, chest, waist, length], i) => (
                              <tr key={size} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                                <td className="py-2 px-3 font-semibold text-primary-700">{size}</td>
                                <td className="py-2 px-3 text-center text-gray-700">{chest}</td>
                      ...
                      <p className="text-xs text-gray-400 mt-4">
                        * Klicken Sie außerhalb oder auf × um zu schließen. Ihre Auswahl bleibt erhalten.
                      </p>
                      <button
                        onClick={() => setShowSizeChart(null)}
