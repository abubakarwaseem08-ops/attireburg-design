...
export function CartProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (!newItem.isBackorder) {
        try {
          ...
          // If stock check fails (non-ok response or network error), we allow the add to proceed
        } catch (stockError) {
          // Only re-throw if it's our own "not enough stock" error
          if (stockError instanceof Error && stockError.message.startsWith('Nur ')) {
            throw stockError
          }
          // Otherwise silently continue — don't block the cart for API issues
          console.warn('Stock check unavailable, proceeding with add:', stockError)
        }
      }
  ...
  const updateQuantity = (id: string, quantity: number) => {
    ...
    setItems(prev =>
      prev.map(item =>
        item.id === id
          ? { ...item, quantity: item.isBackorder ? quantity : Math.min(quantity, item.stock) }
          : item
      )
    )
  }
